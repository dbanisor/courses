...
server_socket.bind(('127.0.0.1', HOST_PORT))
...
def broadcast_message(message):
    '''Send a message to all clients conneted to the server'''
...
def receive_message(client_socket):
    ...
    while True:
        try:
            ...
            message = f"\033[1;92m\t{name}: {message}\033[m".encode(ENCODER)
            ...
            # Remove client socket and name from list
...
def connect_client():
    ...
    while True:
        ...
        client_socket.send(f"{client_name}, you have connected to the server!". encode(ENCODER)) #for individual client
